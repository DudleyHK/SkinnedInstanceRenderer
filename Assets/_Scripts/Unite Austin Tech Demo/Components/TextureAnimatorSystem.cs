...
public class TextureAnimatorSystem : JobComponentSystem
{
    ...
    private ComponentGroup tankUnits;
    private ComponentGroup rangedUnits;
    private ComponentGroup skeletonUnits;
    ...
    {
        units = GetComponentGroup(typeof(TextureAnimatorData), typeof(UnitTransformData));
        meleeUnits = GetComponentGroup(typeof(MeleeUnitData), typeof(TextureAnimatorData), typeof(UnitTransformData));
        tankUnits = GetComponentGroup(typeof(TankUnitData), typeof(TextureAnimatorData), typeof(UnitTransformData));
        rangedUnits = GetComponentGroup(typeof(RangedUnitData), typeof(TextureAnimatorData), typeof(UnitTransformData));
        skeletonUnits = GetComponentGroup(typeof(SkeletonUnitData), typeof(TextureAnimatorData), typeof(UnitTransformData));
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if(perUnitTypeDataHolder != null)
        {
            ...
            ComponentDataArray<TextureAnimatorData> unitTextureAnimationData = units.GetComponentDataArray<TextureAnimatorData>();
            ComponentDataArray<UnitTransformData> unitTransformData = units.GetComponentDataArray<UnitTransformData>();

            ComponentDataArray<TextureAnimatorData> meleeTextureAnimationData = meleeUnits.GetComponentDataArray<TextureAnimatorData>();
            ...
            var prepareAnimatorJob = new PrepareAnimatorDataJob()
            {
                ...
                dt = dt,
                textureAnimatorData = unitTextureAnimationData,
            };

            var prepareAnimatorFence = prepareAnimatorJob.Schedule(units.CalculateLength(), SimulationState.BigBatchSize, previousFrameFence);
            ...
            foreach(var data in perUnitTypeDataHolder)
            {
                ...
                {
                    case UnitType.Melee:
                        ComputeFences(meleeTextureAnimationData, dt, meleeTransformData, data, prepareAnimatorFence, jobHandles, 0);
                        data.Value.Count = meleeUnits.CalculateLength();
                        break;
                    case UnitType.Skeleton:
                        ComputeFences(skeletonTextureAnimationData, dt, skeletonTransformData, data, prepareAnimatorFence, jobHandles, 3);
                        data.Value.Count = skeletonUnits.CalculateLength();
                        break;
                }
            }
